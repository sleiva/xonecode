 ...
 * como RANURA (`cloudstudio`), igual que las otras vistas: esta pestaña decide DÓNDE, no qué.
 */
...
export function Revision({
  ...
  conectado?: boolean;
  /**
   * La banda de arriba: la sincronización con CloudStudio, ya montada por `App`. Va como
   * ranura por lo de siempre —aquí se elige dónde, no qué— y **se pinta en TODOS los estados
   * de abajo**, incluidos los cinco que no tienen lista. `CloudStudio` mide al MONTARSE
   * —«entrar a mirar ES la pregunta», y aquí entrar es abrir esta pestaña—, así que un
   * estado temprano que se llevara por delante la banda dejaría sin pedir el número justo
  ...
  } else if (via === "sin-marca" && historica === true) {
    cuerpo = (
      <p className={estilos.aviso}>
