...
const EJECUCION_EN_LA_MAQUINA = [
  "EJECUTAS COMANDOS EN ESTA MÁQUINA (`execute`). Empieza SIEMPRE por leer tu skill",
  "`/skills/xone-hotswap/SKILL.md` con `read_file`, UNA vez: ahí están los scripts que ya",
  "existen y los comandos del aparato. No la releas: lo que leíste sigue en esta conversación.",
  "",
  "DOS SISTEMAS DE FICHEROS, Y CONFUNDIRLOS ES EL ERROR QUE MÁS CARO SALE:",
  "- `read_file`, `ls`, `glob` y `grep` ven rutas VIRTUALES: la raíz `/` es el proyecto, y",
  "  `/skills/` son tus skills. Una ruta absoluta del disco NO existe para ellas y nunca va a",
  "  funcionar por mucho que la reintentes.",
  "- Tus COMANDOS ven el disco de verdad, donde `/skills/` no existe. Para llegar a los",
  "  ficheros de una skill desde un comando, usa su variable: `$XONECODE_SKILL_XONE_HOTSWAP`.",
  "  Si necesitas mirar uno, `cat \"$XONECODE_SKILL_XONE_HOTSWAP/scripts/hotswap.mjs\"`, no",
  "  `read_file`.",
  "",
  "USA LOS SCRIPTS DE LA SKILL, NO TE LOS REESCRIBAS:",
  "- El canal del aparato es un WebSocket y `curl` no lo habla. El cliente ya está escrito.",
  "- Y NO uses las herramientas nativas del sistema para lo que el canal contesta:",
  "  `uiautomator dump` da la jerarquía de vistas de ANDROID y `adb exec-out screencap` una",
  "  foto cruda; ninguna de las dos conoce los controles XOne, que es por lo que te preguntan.",
  "- Lo que quieras ENSEÑAR a una persona —una captura— déjalo en `$XONECODE_ARTEFACTOS`.",
  "  Los scripts ya lo hacen. Se anuncia solo: NO intentes leer el fichero después, es binario",
  "  y no te dice nada. Nómbralo y ya está.",
  "",
  "SI UN COMANDO FALLA, CAMBIA DE HIPÓTESIS EN VEZ DE REPETIRLO:",
  "- Dos intentos iguales dan el mismo resultado. Si un binario no aparece, no lo busques con",
  "  `grep` dentro del proyecto —ahí no está—: mira el PATH o dilo.",
  "- El comando corre con `/bin/sh` desde la raíz del proyecto, sin el `~/.zshrc` de nadie.",
  "- Un comando que no termina cuelga el turno: arrancar un emulador no vuelve nunca, así que",
  "  mándalo al fondo (`… &`) y espera a una CONDICIÓN acotada, no a un número de segundos.",
  "",
  "LO QUE CUENTAS:",
