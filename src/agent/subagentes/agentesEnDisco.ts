...
/**
 * **El FLUJO y la MAQUETA de cada tipo de documento**, para el que documenta.
 *
 * Sin esto el especialista sabía QUÉ tipos existen —su ficha los nombra— pero no qué lleva cada
 * uno ni en qué orden trabajarlo, así que cada manual salía con otra estructura y el técnico y el
...
const FLUJO_Y_MAQUETAS_DE_DOCUMENTO = [
  ...
  "1. **Tipo y lector.** Del encargo sale QUÉ documento es y PARA QUIÉN. Si no lo dice, elige el",
  "   tipo que mejor encaje, dilo al principio de tu respuesta y sigue: no te pares a preguntar.",
  ...
  "3. **¿Ya hay algo? RETOMA, no rehagas.** Antes de escribir, mira si existe la carpeta del",
  "   documento (`ls /doc/manual-usuario/`). Si hay secciones, es que un encargo anterior se cortó",
  "   a medias: léete el índice, NO reescribas las que ya están y escribe desde la primera que",
  "   falte. Rehacerlas gasta lo mismo otra vez y deja números repetidos.",
  "4. **Escribe por SECCIONES, un fichero pequeño cada una**, en una carpeta con el nombre del",
  ...
  "5. **Une con `unir_secciones`** al terminar: el documento queda en `/doc/manual-usuario.md` sin",
  "   que lo reescribas. Para corregir algo, edita SU sección y vuelve a unir; no edites el",
  "   documento unido, que la unión siguiente lo pisa. Para RETIRAR una sección, vacíala: una",
  "   vacía no entra. Si `unir_secciones` avisa de números repetidos, arréglalo antes de cerrar.",
  "   Si no tienes `unir_secciones`, escribe un solo fichero y ve añadiendo cada sección con",
  "   `edit_file`.",
  "6. **Solo lo que está en el proyecto.** Lo que no puedas comprobar se marca «(sin comprobar)»;",
  "   no se inventa una función, un campo ni un flujo.",
  "7. **Cierra** con la ruta del documento y, aparte, lo que encontraste mal.",
  "",
  ...
  "",
  "## Maqueta por tipo",
  "**Manual de usuario** — para quien USA la app, sin jerga: nunca «colección», «nodo» ni «script».",
  "Nombra las pantallas y los campos como los ve el usuario (su título visible, no su nombre interno).",
  "1. Qué es la app y para qué sirve (un párrafo).",
  "2. Acceso: cómo se entra y el login, si lo hay.",
  "3. Una sección POR PANTALLA o flujo, en el orden en que se usan desde la entrada: para qué",
  "   sirve, cómo se llega, pasos numerados, captura, y los campos que ve con lo que significan.",
  "4. Mensajes y errores habituales, y qué hacer con cada uno.",
  "5. Preguntas frecuentes y glosario.",
  "",
  "**Manual técnico** — para quien MANTIENE o amplía la app: aquí sí nombres internos y rutas.",
  "1. Visión general y arquitectura, con un diagrama de pantallas y navegación.",
  "2. Estructura del proyecto: qué hay en cada carpeta y fichero relevante.",
  "3. Arranque: entrada, login y estilos (de `xone_navegacion app`).",
  "4. Modelo de datos: una tabla por colección con sus campos, tipos y relaciones",
  "   (`mapcol`, `linkedfield`, `contents`, `inherits`).",
  "5. Navegación: qué pantalla abre cuál, y desde dónde (botones, scripts).",
  "6. Lógica: scripts, funciones y eventos, con qué hace cada uno y dónde está.",
  "7. Integraciones y conexiones con servicios externos.",
  "8. Estilos: hojas CSS y clases principales.",
  "9. Configuración y despliegue, y problemas conocidos (`xone_navegacion problemas`).",
  "",
  "**Arquitectura** — piezas y relaciones, más que detalle: diagrama general, componentes y su",
  "responsabilidad, flujo de datos entre pantallas y servicios, y las decisiones que se ven en el",
  "código con su porqué si consta.",
  "",
  "**Seguridad** — autenticación y login, qué datos sensibles guarda y dónde, permisos, conexiones",
  "externas y cómo viajan los datos, y los riesgos que veas, cada uno con dónde está en el código.",
  "",
  "**Notas de versión** — para quien actualiza: novedades, cambios, correcciones y lo que rompe",
  "compatibilidad, en frases cortas. Sale de lo que cambió (el encargo o el historial), no de todo",
  "el proyecto.",
  "",
  "Si piden otro tipo —guía de instalación, documento funcional para el cliente—, arma una maqueta",
  "con el mismo criterio: lector, índice primero, y solo lo que está en el proyecto.",
].join("\n");
...
const EJECUCION_EN_LA_MAQUINA = [
  ...
  "  varios comandos seguidos. Pero `--captura` NO es gratis por ir en el mismo viaje: una",
  "  captura cuesta 3.375 fichas en cuanto alguien la mira, así que se pide cuando quieres VER",
  "  algo, no para diagnosticar — un fallo de arranque lo cuenta el log con fichero y línea.",
  ...
  "  lo que solo necesites RELEER —un árbol de controles, un log— a `$XONECODE_HOTSWAP`, que no",
  "  sale en el chat y se abre con `read_file` en `/hotswap/<nombre>`. Los scripts ya reparten",
  "  por ti. Una captura es `xone-hotswap shot` y, si el canal no contesta,",
  "  `xone-captura-android` — nunca un `adb … screencap > fichero`, que el `cwd` es la raíz",
  "  del proyecto.",
  "- **Los comandos se escriben `<nombre> clave=valor`, NUNCA en JSON.** `xone-hotswap click",
  "  name=BTN_OK`, y con `--` se encadenan varios por la MISMA conexión (el servidor admite",
  "  una sola y cierra la anterior). Un JSON se rechaza diciéndote la forma buena, pero eso",
  "  es un viaje perdido.",
  "- **ELIGE LA HERRAMIENTA ANTES DE TECLEAR, que no cuestan lo mismo:**",
  "  · Para saber DÓNDE estás: `screen`. Una línea, con la colección activa y los diálogos",
  "    abiertos. Encadénalo tras cada navegación (`click name=X -- screen`).",
  "  · Para saber QUÉ FALLA: el LOG (`xone-log-android`). Un error de JavaScript, una",
  "    excepción o una colección que no carga salen ahí con su mensaje y su línea.",
  "  · Para LEER valores: `getText` uno, `getFields names=A,B,C` varios en un viaje,",
  "    `getRows content=X fields=…` los de una lista entera.",
  "  · `elements` SOLO para explorar cuando aún no sabes qué mirar: es un volcado de decenas",
  "    de miles de caracteres y se te queda en el contexto el resto de la sesión.",
  "- **Una CAPTURA no es una herramienta de diagnóstico.** Para averiguar por qué algo falla",
  "  están el log y `elements`; la captura es para lo VISUAL —que algo se ve cortado, tapado o",
  "  ilegible— o para cuando lo anterior no concluye. MEDIDO: una captura de pantalla completa",
  "  cuesta 3.375 tokens en el revisor visual, y el peso del fichero da igual (52 KB y 766 KB",
  "  cuestan lo mismo: se paga por píxeles). Acótala a lo que compruebas —`shot name=MAP_X`—,",
  "  que además dice QUÉ se estaba probando en vez de ser una foto de la pantalla.",
  "- **El aparato lo dice el encargo**: cada turno llega con una línea `[Dispositivo de esta",
  "  sesión: …]`. Es el que la persona eligió en el chat, y los scripts `xone-*` ya lo usan solos",
  "  —no pases otro `--serie`/`--udid` si no te lo piden—. Sin elección, prefieren un EMULADOR a",
  "  un dispositivo físico. Cada script dice por stderr con cuál trabajó.",
  ...
  "- **NO añadas `sleep` después de desplegar o lanzar: los scripts YA esperan.**",
  "  `xone-desplegar-android` sondea el canal y luego la app —hasta 15 s y hasta 20 s— y",
  "  SALE en cuanto contestan, así que un `sleep 12` detrás es tiempo muerto que nadie",
  "  aprovecha. Y para esperar a un control está `waitForElement name=X timeout=5000`, que",
  "  espera con la semántica del framework y te dice por qué falla si se agota. Un número",
  "  de segundos o sobra o se queda corto, y las dos cosas cuestan.",
  "",
  "LO QUE CUENTAS:",
