...
const SIN_DISCO = { write: () => ({ error: "sin disco" }) };
...
describe("las tools propias de xonecode, adaptadas a TrueForge", () => {
  ...
  it("el esquema es el de la tool de LangChain, en JSON Schema", async () => {
    const { result } = await fuenteDeLangchain([navegacion], SIN_DISCO).listTools();
  ...
  it("la llamada es su `invoke`: la MISMA respuesta que en deepagents, callejón incluido", async () => {
    const r = (await fuenteDeLangchain([navegacion], SIN_DISCO).callTool({ name: "xone_navegacion", arguments: { operacion: "inventario" } })) as unknown as Respuesta;
  ...
  it("un argumento que no pasa el esquema se le DEVUELVE al modelo, no tumba el turno", async () => {
    const r = (await fuenteDeLangchain([navegacion], SIN_DISCO).callTool({ name: "xone_navegacion", arguments: { operacion: "inventada" } })) as unknown as Respuesta;
