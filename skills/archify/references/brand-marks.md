...
> **Ningún nodo que monta esta skill tiene `execute`.** Todos los comandos de este fichero
> (`node bin/archify.mjs brands …`) son HISTÓRICOS y no se pueden correr desde aquí — sin un
> brand ya conocido en el esquema, omite `brand` (ver `../SKILL.md`).
