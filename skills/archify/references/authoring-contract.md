...
`flowchart`/`graph` → `workflow` (o `architecture` para un mapa de componentes);
`sequenceDiagram` → `sequence`; `stateDiagram` → `lifecycle`. Lee la topología y el significado,
después autora JSON fresco de Archify — nunca copies mecánicamente el estilo o las coordenadas
de Mermaid.
